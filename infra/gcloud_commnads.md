...
### 10.1 呼び出し用SA（例）
Scheduler専用のSAを作る（推奨）:

```bash
gcloud config set project ikeuchi-data-sync

gcloud iam service-accounts create sa-review-observation-scheduler \
...
export REGION="asia-northeast1"
export SERVICE_NAME="review-observation"
export SCHEDULER_SA="sa-review-observation-scheduler@ikeuchi-data-sync.iam.gserviceaccount.com"
