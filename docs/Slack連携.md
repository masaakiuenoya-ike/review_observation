...
- **「hourly が動いているのに、なぜ 09:15 はコールドなのか」**  
  hourly の POST / は**リクエストが来てから 1〜2 分で終了**する。終了後、次の hourly まで**約 58 分はリクエストが来ない**。Cloud Run は無負荷が 15〜30 分続くとインスタンスを 0 にするため、「毎時数分だけアクティブ」ではスケールゼロを防げない。その結果、09:15 の daily-summary がコールドで当たり、起動に時間がかかって 000（クライアント側タイムアウト）になりやすい。
