...
def main() -> None:
    ...
    if not code_holder:
        print("認証コードを取得できませんでした。ブラウザで URL を開き、同意後に localhost:8080 にリダイレクトされたか確認してください。", file=sys.stderr)
        sys.exit(1)
    ...
    if not refresh:
        print("レスポンスに refresh_token が含まれていません。prompt=consent で再試行するか、既に同意済みの場合は一度アプリのアクセスを解除してから再度実行してください。", file=sys.stderr)
